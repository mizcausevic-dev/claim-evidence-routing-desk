...
export function toSummary(report: CoverageReport) {
  ...
  return `${posture} ${report.claims} claims · ${report.packets} packets · ${report.highSeverityPackets} high-severity packets · ${report.workflowGaps} workflow gaps`;
}
...
export function toMarkdown(report: CoverageReport) {
  ...
  return [
    `# Claim evidence posture ${report.ok ? "✅" : "❌"}`,
    "",
    `- Claims: **${report.claims}**`,
    `- On-track claims: **${report.onTrackClaims}**`,
